				...
				if (tier.containsKey(firstPlayer) && tier.containsKey(secondPlayer)) {
					...
					for (Map.Entry<String, Integer> entry : test.entrySet()) {
						if (tier.get(secondPlayer).containsKey(entry.getKey())) {
							int firstPlayerSkill = tier.get(firstPlayer).values().stream().reduce((v1, v2) -> v1 + v2)
									.get();
							int secondPlayerSkill = tier.get(secondPlayer).values().stream().reduce((v1, v2) -> v1 + v2)
									.get();
